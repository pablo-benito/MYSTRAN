 ...
 * </pre>
 */
int_t dLUMemInit(fact_t fact, void *work, int_t lwork, int m, int n, int_t annz,
                 int panel_size, double fill_ratio, SuperMatrix *L,
                 SuperMatrix *U, GlobalLU_t *Glu, int **iwork, double **dwork) {
 ...
 */
int dLUWorkInit(int m, int n, int panel_size, int **iworkptr, double **dworkptr,
                GlobalLU_t *Glu) {
 ...
 */
void dSetRWork(int m, int panel_size, double *dworkptr, double **dense,
               double **tempv) {
  double zero = 0.0;
...
int_t dLUMemXpand(
  ...
  printf("dLUMemXpand[1]: jcol %d, next %lld, maxlen %lld, MemType %d\n", jcol,
         (long long)next, (long long)*maxlen, mem_type);
#endif
 ...
 */
void *
dexpand(int_t *prev_len,   /* length used from previous call */
...
int_t dmemory_usage(const int_t nzlmax, const int_t nzumax, const int_t nzlumax,
                    const int n) {
  ...
  return (10 * n * iword + nzlmax * liword + nzumax * (liword + dword) +
          nzlumax * dword);
}
